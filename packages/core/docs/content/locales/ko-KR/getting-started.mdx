...
<FileTree
  ...
  title="chat-first app 형태"
  entries={[
    {
      path: "app/routes/_index.tsx",
      note: "사용자가 처음 시작하는 전체 페이지 chat surface",
    },
    {
      path: "actions/analyze-responses.ts",
      note: "agent, UI, CLI, HTTP, MCP, A2A가 호출할 수 있는 typed operation",
    },
    {
      path: "server/db/schema.ts",
      note: "agent가 추가하거나 업데이트할 수 있는 persistent Response insights data",
    },
    {
      path: "app/routes/response-insights.tsx",
      note: "chat result 뒤의 data를 살펴보는 durable app page",
    },
    {
      path: "actions/view-screen.ts + actions/navigate.ts",
      note: "agent가 올바른 screen을 열 수 있게 하는 context와 navigation actions",
    },
  ]}
/>
...
## 고급: 아직 브라우저 UI가 필요 없을 때 {#no-browser-ui}

예약 리포트, queue, CLI loop, integration worker, 외부 agent 호출은 no-browser scaffold를 사용할 수 있습니다.

```bash
npx @agent-native/core@latest create my-agent --headless
```

이것도 actions, sessions, run history, settings, credentials, SQL state를 가진 Agent-Native app입니다. automation-first 형태일 뿐 기본 시작 경로는 아닙니다. [Automation-First Apps](/docs/pure-agent-apps)를 참조하세요.
