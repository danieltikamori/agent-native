...
Slack 私信只有在验证电子邮件并确认用户属于 Agent Native 组织后，才会使用用户身份运行。共享频道使用服务主体，不会借用参与者的私人权限。托管 OAuth 和生成的 Slack manifest 都会请求 `users:read.email`；已有安装在添加该 scope 后必须重新连接，旧 bot token 则必须在 Slack 中手动更新。
