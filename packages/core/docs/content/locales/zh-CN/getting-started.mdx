...
<FileTree
  ...
  entries={[
    ...
    },
    {
      ...
      note: "agent、UI、CLI、HTTP、MCP 和 A2A 都能调用的 typed operation",
    },
    {
      path: "server/db/schema.ts",
      note: "agent 可以添加或更新的持久 Response insight 数据",
    },
    {
      path: "app/routes/response-insights.tsx",
      note: "用于浏览聊天结果背后数据的持久 app 页面",
    },
    {
      path: "actions/view-screen.ts + actions/navigate.ts",
      note: "让 agent 可以打开正确屏幕的 context 和 navigation actions",
    },
  ]}
/>
...
## 高级：暂时不需要浏览器 UI {#no-browser-ui}

定时报告、队列、CLI loop、integration worker 或外部 agent 调用这类工作，可以使用 no-browser scaffold：

```bash
npx @agent-native/core@latest create my-agent --headless
```

这仍然是 Agent-Native app：actions、sessions、run history、settings、credentials 和 SQL state 都存在。它只是 automation-first 形态，不是默认入门路径。见 [Automation-First Apps](/docs/pure-agent-apps)。
