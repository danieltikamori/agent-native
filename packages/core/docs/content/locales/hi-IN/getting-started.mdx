...
<FileTree
  ...
  title="chat-first app shape"
  entries={[
    {
      path: "app/routes/_index.tsx",
      note: "full-page chat surface जहां users शुरू करते हैं",
    },
    {
      path: "actions/analyze-responses.ts",
      note: "typed operation जिसे agent, UI, CLI, HTTP, MCP और A2A call कर सकते हैं",
    },
    {
      path: "server/db/schema.ts",
      note: "persistent Response insights data जिसे agent add या update कर सकता है",
    },
    {
      path: "app/routes/response-insights.tsx",
      note: "chat result के पीछे data browse करने वाला durable app page",
    },
    {
      path: "actions/view-screen.ts + actions/navigate.ts",
      note: "context और navigation actions ताकि agent सही screen खोल सके",
    },
  ]}
/>
...
## Advanced: अभी browser UI नहीं चाहिए {#no-browser-ui}

Scheduled reports, queues, CLI loops, integration workers या दूसरे agent की calls no-browser scaffold इस्तेमाल कर सकती हैं:

```bash
npx @agent-native/core@latest create my-agent --headless
```

यह फिर भी actions, sessions, run history, settings, credentials और SQL state वाली Agent-Native app है. यह automation-first shape है, मुख्य getting-started path नहीं. देखें [Automation-First Apps](/docs/pure-agent-apps).
