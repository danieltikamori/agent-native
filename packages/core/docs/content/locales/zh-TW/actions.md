...
```an-annotated-code title="動作剖析"
{
  ...
  "annotations": [
    { "lines": "5", "label": "工具表面", "note": "`description` 是代理讀取以決定何時呼叫此動作的內容。每個欄位的 `.describe()` 也會進入 JSON Schema。" },
...
```an-api title="自動掛載的 action 端點" method="GET" path="/_agent-native/actions/get-lead"
{
  ...
  "summary": "每個 action 都會自動掛載在這裡 - 檔名就是 action 名稱。",
  "description": "預設是 POST；`http: { method: \"GET\" }` 會讓它成為 GET。無論任何 `http.path` 覆蓋如何，React 鉤子 和 `callAction` 始終按名稱呼叫這個路徑。",
  "auth": "工作階段 cookie；前端呼叫會攜帶 `X-Agent-Native-Frontend: 1`",
  "params": [
