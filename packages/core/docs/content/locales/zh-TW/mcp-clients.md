...
```an-annotated-code title="mcp.config.json，帶註解"
{
  ...
  "annotations": [
    ...
    { "lines": "4-6", "label": "stdio launcher", "note": "`command` + `args` 生成本機二進制檔案。 Stdio 伺服器旨在用於**本機開發**- 它們在邊緣執行時中是無操作的。" },
    { "lines": "6", "label": "行程環境", "note": "Optional `env` is passed to the spawned process. Keep secrets out of committed config; prefer `MCP_SERVERS` or the settings UI for tokens." }
  ]
}
```
