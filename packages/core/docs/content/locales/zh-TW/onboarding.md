...
```an-annotated-code title="註冊自訂入門步驟"
{
  ...
  "annotations": [
    { "lines": "5", "label": "自動安裝", "note": "從 Nitro 外掛註冊 - 此框架處理渲染、完成跟蹤和解除。" },
    { "lines": "7", "label": "穩定ID", "note": "預設載入後使用相同的 `id` 重新註冊會覆蓋內建步驟。" },
    ...
    { "lines": "28-31", "label": "竣工檢查", "note": "`isComplete` 在伺服器端執行。 OAuth 權杖位於 `oauth_tokens` 儲存中 - 檢查它，而不是 `process.env.GMAIL_REFRESH_TOKEN`。" }
  ]
}
```
