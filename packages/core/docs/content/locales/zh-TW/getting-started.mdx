...
<FileTree
  ...
  entries={[
    ...
    },
    {
      ...
      note: "agent、UI、CLI、HTTP、MCP 與 A2A 都能呼叫的 typed operation",
    },
    {
      path: "server/db/schema.ts",
      note: "agent 可以新增或更新的持久 Response insight 資料",
    },
    {
      path: "app/routes/response-insights.tsx",
      note: "用於瀏覽聊天結果背後資料的持久 app 頁面",
    },
    {
      path: "actions/view-screen.ts + actions/navigate.ts",
      note: "讓 agent 可以開啟正確畫面的 context 和 navigation actions",
    },
  ]}
/>
...
## 進階：暫時不需要瀏覽器 UI {#no-browser-ui}

排程報告、佇列、CLI loop、integration worker 或外部 agent 呼叫這類工作，可以使用 no-browser scaffold：

```bash
npx @agent-native/core@latest create my-agent --headless
```

這仍然是 Agent-Native app：actions、sessions、run history、settings、credentials 與 SQL state 都存在。它只是 automation-first 形態，不是預設入門路徑。見 [Automation-First Apps](/docs/pure-agent-apps)。
