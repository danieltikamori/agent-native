...
<FileTree
  ...
  title="The chat-first app shape"
  entries={[
    {
      path: "app/routes/_index.tsx",
      note: "the full-page chat surface users start from",
    },
    {
      path: "actions/analyze-responses.ts",
      note: "a typed operation the agent, UI, CLI, HTTP, MCP, and A2A can call",
    },
    {
      path: "server/db/schema.ts",
      note: "persistent response-insight data the agent can add or update",
    },
    {
      path: "app/routes/response-insights.tsx",
      note: "a durable app page for browsing the data behind the chat result",
    },
    {
      path: "actions/view-screen.ts + actions/navigate.ts",
      note: "context and navigation actions so the agent can open the right screen",
    },
  ]}
/>
...
## Advanced: no browser UI yet {#no-browser-ui}

Some workflows really do not need a browser surface at first: scheduled reports,
queues, integration workers, CLI loops, or another app calling this app over
A2A/MCP. For those cases, use the no-browser scaffold:

```bash
npx @agent-native/core@latest create my-agent --headless
```

That is still an Agent-Native app with actions, sessions, run history, settings,
credentials, and SQL state. It is not the main getting-started path because most
agentic products benefit from chat, inline results, and eventually durable
screens. See [Automation-First Apps](/docs/pure-agent-apps) for the no-browser
pattern.
